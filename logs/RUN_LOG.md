...
日期：2026-04-13
任务名称：地图修复 A+B — 中国台湾/海南坐标 + 捷克形状
Agent 名称：主控 Agent
输入：
  - 中国 extra_islands 坐标错误（台湾在北部、海南超出地图北边界）
  - 捷克形状不够像真实轮廓（16 tile 太少）
输出：
  - china.json: extra_islands 修正 — 台湾 [[7,1],[7,2]]（东南，gap=2）、海南 [[0,5],[1,5]]（正南，gap=2）
  - china.json: 对应港口更新 — 台湾 (7,1)side0、海南 (0,5)side5
  - rasterizer.py: czech_republic budget 16→22，产出 21T 更清晰展现波西米亚+摩拉维亚形状
验证方式：PNG 渲染确认台湾右下/海南正下位置；151 pytest passed；e2e_smoke 14/14
发现的问题：extra_islands 坐标是相对于 rasterized+recentered 空间，不是地理坐标，需要先查 mainland tile 范围再反推
教训：坐标系容易踩坑 — 调试时先 print 实际 tile bbox，再对照地理位置手动推算 q/r
...
日期：2026-04-13
任务名称：前端 Czech Republic 地图注册 + 中国地图台湾/海南岛缝隙
Agent 名称：主控 Agent
输入：
  - 会话开始：frontend/src/maps/definitions.ts 有未提交的 czech_republic 前端注册 diff；PostgreSQL 未启动（Connection refused 5432）
  - 用户："继续"（续上会话，补提交 definitions.ts + 查看可继续的任务）
  - 用户："地图之间可以有缝隙，比如中国大陆和海南还有台湾，可以改一下的"
输出：
  commit 6ff62ee: feat(maps): register Czech Republic in frontend MAP_CONFIGS
    - frontend/src/maps/definitions.ts: 追加 czech_republic preview tile grid
    - e2e_smoke 14/14 验证后提交
  commit ae9506a: feat(maps): add Taiwan + Hainan islands to China map with visible sea gaps
    - 根因分析：台湾海峡仅 ~1° 经度宽，中国大陆面积是台湾 600 倍；在任何 <100 tiles 比例下，海峡不够放 1 个 hex gap（verified: 需 budget=90+ 才能让 scale 小到台湾出现 1 tile，但仍会立刻贴近大陆）
    - 方案：新增通用 `extra_islands` 机制——polygon JSON 可声明 hex 坐标，build_map 在大陆 rasterize + recenter 后注入
    - backend/app/maps/rasterizer.py: build_map 新增 extra_islands 注入循环（9 行）
    - backend/app/maps/data/polygons/china.json: 新增 extra_islands 字段：台湾 [(6,-3),(6,-4)]、海南 [(0,-4),(1,-4)]
    - 两岛与大陆 min gap = 2 hexes（代码验证）；总计 60 tiles / 3 组件 [56,2,2]
    - 157 pytest passed
置信度：高
卡住了吗：否
验证方式：
  - 第一层：pytest 157 passed
  - 第二层：e2e_smoke 14/14（PostgreSQL brew services start postgresql@15 启动后）
  - island gap 代码验证：_axial_distance 全量比对 min=2
  - PNG 视觉确认（render_maps_png.py）：3 个独立组件清晰可见
发现的问题：
  - MULTI_ISLAND_MAPS 中的 'philippines' 无对应 polygon JSON（缺文件），但 philippines 不在 _SLUGS 故不影响线上
  - PostgreSQL 版本为 15，非 14，startup 脚本需用 brew services start postgresql@15
备注：
  - `extra_islands` 是通用机制，未来 UK（Channel Islands）、Italy（Sardinia 更多定制）等也可使用
  - 台湾位置 (6,-3)(6,-4) 对应游戏板右侧中部，海南 (0,-4)(1,-4) 对应底部中央，符合"东南偏海"直觉
